...
namespace PCMDFT
{
    ...
    DFTThread::DFTThread (std::shared_ptr<const PCMSettings> spSettings, QObject* parent, std::size_t clPlatId, std::size_t clDeviceId) :
        QObject {parent}, spThread_ {new QThread}, spSettings_ {spSettings}, spCLData_ {new CLData},
    clPlatId_ {clPlatId}, clDeviceId_ {clDeviceId}
    {
    ...
    void DFTThread::slotTimeSeriesUpdate (QByteArray bytes)
    {
        try
        {
            ...
            for (std::size_t i = 0; i < buf.size1(); ++i)
            {
                ...
                std::size_t szLocal {spCLData_->spKernel_->getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE> (spCLData_->devices_.at (clDeviceId_)) };
                ...
                cl::CommandQueue queue {*spCLData_->spContext_, spCLData_->devices_.at (clDeviceId_), CL_QUEUE_PROFILING_ENABLE};
